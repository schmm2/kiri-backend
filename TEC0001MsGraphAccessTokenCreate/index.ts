...
const TEC0001MsGraphAccessTokenCreate: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    if (payload.tenantId && payload.appId) {
        ...
        try {
            retrievedSecret = await client.getSecret(payload.appId);
        }
        catch (error) {
            console.log(error);
        }
...
export default TEC0001MsGraphAccessTokenCreate;